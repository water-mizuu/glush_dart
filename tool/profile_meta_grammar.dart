...
void main() {
  ...
  var ast = measure('GrammarFileParser.parse(meta) once for compile', 1, () => GrammarFileParser(metaGrammarString).parse());
  measure('GrammarFileCompiler.compile(meta)', 30, () => GrammarFileCompiler(ast).compile(startRuleName: 'full'));
  var grammar = measure('compile+SMParserMini ctor', 10, () {
    var g = GrammarFileCompiler(GrammarFileParser(metaGrammarString).parse()).compile(startRuleName: 'full');
    return SMParserMini(g);
  });
  measure('metaParser.parse(simple)', 50, () => grammar.parse("rule = 'a'\n"));
  measure('metaParser.parse(self)', 5, () => grammar.parse(metaGrammarString));
}
